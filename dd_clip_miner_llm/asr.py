...
class Transcriber:
    def __init__(self, config: dict[str, Any], *, asr_dir: str | Path | None = None) -> None:
        self.settings = config["asr"]
        runtime_context = {"asr_dir": Path(asr_dir)} if asr_dir is not None else None
        self._backend = build_asr_backend(self.settings, runtime_context=runtime_context)
